...
**Dossier** is the opposite: an **autonomous adversarial idea intelligence platform** built on the **TrueForge Agent Harness**. It deploys **6 specialized subagents** to aggressively stress-test concepts, cross-examine assumptions, verify live prediction market consensus via Polymarket, execute sandboxed Python unit economics simulations, and halt at human approval checkpoints before executing real-world actions.
...
### 🏆 Grand Prize ($1,500)
* Full end-to-end multi-agent platform combining live LLM routing, live prediction market oracle, community recon, isolated sandbox execution, and an intelligence terminal UI.
...
* **Adversarial Cross-Examination:** Subagents directly challenge and debate conflicting hypotheses (e.g. Red Team vs Investor).
* **Contradiction Surface:** Surfacing unresolved conflicts instead of averaged consensus.

### 💎 Best Code Quality Track ($1,000 Mac Mini via Qodo AI)
* **100% Pull Request Driven Development:** All features merged via Pull Requests with continuous automated **Qodo AI** code reviews on GitHub (`PR #1` through `PR #12`).
* **Strict TypeScript & Node 22 ESM:** NodeNext module resolution, zero build errors, numeric exit codes, and comprehensive automated test suites (`test/*.test.mjs`).
